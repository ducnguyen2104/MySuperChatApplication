...
public class ConversationViewModel extends ViewModel {
    ...
    public ConversationViewModel(Context context) {
        ...
        membersString = ConversationActivity.membersString; //!= null is conversation has 2 or more members
        ...
        conversationRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                } else { //conversation is not created yet
                    ConversationActivity.isConversationCreated = false;
    ...
    public void onSendBtnClicked() {
        ...
        if (message.get().length() != 0) {
            ...
            //create new conversation
                ...
                key = ConversationActivity.currentCId;
                if (!isGroupChat) { //conversation 2 members
                    String anotherUser = conversationName.get();
                    ArrayList<String> users = new ArrayList<>();
                    users.add(anotherUser);
                    users.add(MainActivity.currentUser.getUsername());
                    Collections.sort(users);
                    final Conversation conversation = new Conversation(users.get(0) + "*" + users.get(1), newMsg, users.get(0) + " and " + users.get(1), users);
                    Log.i("newConversation", conversation.toString());
                    //update local
                                ...
                                public void onSuccess(Conversation conversation1) {
                                    ...
                                    newSingle.observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<ArrayList<ConversationItem>>() {
                                        ...
                                        public void onSuccess(ArrayList<ConversationItem> conversationItems) {
                                            ...
                                            HistoryFragment.conversationsItem = conversationItems;
                                            ConversationActivity.isConversationCreated = true;
                    ...
                    Log.i("new gr chat", "users: " + users);
                    if(key == null || key.length() == 0) {
                        key = MainActivity.database.child("conversations").push().getKey();
                        ConversationActivity.currentCId = key;
                    }
                    Conversation conversation = new Conversation(key, newMsg, membersString.replace("*", ", "), users);
                    Log.i("new gr chat", "conv: " + conversation.toString());
                    //update local
                    Single<Conversation> single = Single.just(conversation);
                    single.subscribeOn(Schedulers.io())
                            .observeOn(Schedulers.io())
